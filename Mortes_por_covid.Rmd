...
output: 
  html_document:
    ...
    theme: united
---
#### `r paste("Atualizado em", format(Sys.Date(), '%d-%b-%Y'))`.
...
#### Não há mistério ou fórmula secreta. Se um aluno lê e escreve muito tenderá a ter boas notas em Redação. Se faz exercícios de Matemática, se sairá bem na matéria. Quando faz deveres de casa com afinco, fica acima da média. Se é relapso, fica abaixo.  Com a pandemia de Covid ocorre o mesmo. Se o país faz isolamento social, vacina, higieniza as mãos e usa máscara, a pandemia será atenuada, do contrário, mais luto do que o esperado.
...
#### Porém, mesmo diante de tal tragédia, a forma com reagem os países faz muita diferença. Podemos atenuá-la ou agravá-la. Por isso entendo que morre-se de Covid de 2 maneiras.
#### Do total de mortes uma parte é "de Covid", quando somos atingidos de forma letal pelo vírus; o restante é "por exposição desnecessária à Covid", quando o país não promove suficientemente as medidas para diminuir a circulação do vírus.
...
#### A tabela abaixo indica, em relação a todos os países, a diferença entre a média por milhão e o número de mortes por milhão do país, se foi o caso de ter matado por Covid ou salvado e a quantidade de habitantes mortos ou salvos. 
#### Em nosso país, acrescentamos `r mortes_brasil` cruzes à nossa história.

```{r, echo=FALSE, message=FALSE, fig.height=4, fig.width=9}

mort_pop_tidy %>% 
  select(continente, pais, dif_media_mortes_por_milhao,
         matou_ou_salvou, resultado) %>% 
  datatable(filter = 'top', caption = htmltools::tags$caption(
    ...
    'fonte:', htmltools::em('https://www.worldometers.info/coronavirus/#countries')),
              ...
              colnames = c("Continente", "País",
                           "Diferença entre a média de mortes no mundo e mortes por milhão do país",
                           "Matou ou salvou?", "Resultado"),
              options = list(bPaginate = TRUE)) %>%
  formatCurrency(c(3,5),
  currency = "",
  interval = 3,
  mark = ".",
  digits = 0,
  ...
  before = TRUE) %>%
  formatStyle(columns = 1:5, fontSize = '150%')
...
>#### __Conclusão:__ Em todos cenários o Brasil "matou por Covid", e muito. Não fez o dever de casa. Como um estudante negligente, o Brasil repetirá de ano, década, século.   